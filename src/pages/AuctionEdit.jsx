...
const AuctionWrite = () => {
  ...
  const editPost = async () => {
    const response = await axios.get(
      `http://localhost:8080/auction/${id}`
    );
  ...
  useEffect(() => {
    if (postContent && postContent.furniture) {
      console.log("postContent", postContent);

      form.setFieldsValue({
        categId: categList[postContent.categoryId],
        title: postContent.title,
        desc: postContent.desc,
        price: postContent.startBid,
        width: postContent.furniture.width,
        height: postContent.furniture.height,
        depth: postContent.furniture.depth,
      });
    }
  }, [postContent]);
